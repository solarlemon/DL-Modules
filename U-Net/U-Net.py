...
class UNet(nn.Module):
    def __init__(self,
        ...
        # 编码器的第2个双卷积层，先下采样最大池化使得特征图高和宽减半，然后通道数翻倍         
        self.down1 = Down(base_c, base_c * 2)
        
        # 编码器的第3个双卷积层，先下采样最大池化使得特征图高和宽减半，然后通道数翻倍       
        self.down2 = Down(base_c * 2, base_c * 4)
        
        # 编码器的第4个双卷积层，先下采样最大池化使得特征图高和宽减半，然后通道数翻倍        
        self.down3 = Down(base_c * 4, base_c * 8)
    
        # 编码器的第5个双卷积层，先下采样最大池化使得特征图高和宽减半，然后通道数翻倍        
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        return {"out": logits,"x5":x5}
    
# 假设输入图像大小为 256x256，通道数为 1（例如灰度图像）
input_tensor = torch.randn(1, 1, 256, 256)  # Batch size 1, Channels 1, Height 256, Width 256
...
# 输出结果是一个字典，其中包含键 'out' 和对应的模型输出
output_tensor = outputs['out']

# 输出结果张量的形状
# print(f"output_tensor.shape:{output_tensor.shape}") 
# 如果你想要确保模型能够在GPU上运行（如果你有GPU的话），可以将模型和输入数据移动到GPU上
if torch.cuda.is_available():
    ...
    output_tensor = outputs['out']
    x5 = outputs['x5']
    print(f"最终的下采样x5.shape:{x5.shape}")
    print(f"output_tensor.shape:{output_tensor.shape}")   # 输出结果应该为 [1, 2, 256, 256]（长宽与输入的形状相同）